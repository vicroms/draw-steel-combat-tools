...
export const applyTriggeredActions = async (mode = null, silent = false) => {
  ...
  if (resolvedMode === 'TARGETED' && !targetedIds.size) {
    if (!silent) ui.notifications.warn(game.i18n.localize('DSCT.notice.ta.noTargetForTargeted'));
    return;
  }
  ...
  if (!silent) {
    ...
    ui.notifications.info(game.i18n.format('DSCT.notice.ta.active', { mode: modeLabel }));
  }
};
